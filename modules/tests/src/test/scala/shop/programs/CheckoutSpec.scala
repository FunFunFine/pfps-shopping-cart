...
final class CheckoutSpec extends PureTestSuite {
  ...
  def recoveringClient(attemptsSoFar: Ref[IO, Int],
                       paymentId: PaymentId): PaymentClient[IO] =
    new PaymentClient[IO] {
  ...
  test("empty cart") {
    ...
    forAll {
      (uid: UserId, pid: PaymentId, oid: OrderId, card: Card) =>
        IOAssertion {
          new CheckoutProgram[IO](successfulClient(pid),
                                  emptyCart,
  ...
  test("unreachable payment client") {
    forAll {
      (uid: UserId, oid: OrderId, ct: CartTotal, card: Card) =>
        IOAssertion {
          Ref.of[IO, List[String]](List.empty).flatMap {
            logs =>
              implicit val bg = shop.background.NoOp
              implicit val logger = shop.logger.acc(logs)
              new CheckoutProgram[IO](unreachableClient,
                                      successfulCart(ct),
                                      successfulOrders(oid),
                                      retryPolicy)
                .checkout(uid, card)
    ...
    forAll {
      (uid: UserId, pid: PaymentId, oid: OrderId, ct: CartTotal, card: Card) =>
        IOAssertion {
          Ref.of[IO, List[String]](List.empty).flatMap {
            logs =>
              Ref.of[IO, Int](0).flatMap {
                ref =>
                  implicit val bg = shop.background.NoOp
                  implicit val logger = shop.logger.acc(logs)
                  new CheckoutProgram[IO](recoveringClient(ref, pid),
                                          successfulCart(ct),
                                          successfulOrders(oid),
                                          retryPolicy)
                    .checkout(uid, card)
  ...
  test("cannot create order, run in the background") {
    forAll {
      (uid: UserId, pid: PaymentId, ct: CartTotal, card: Card) =>
        IOAssertion {
          Ref.of[IO, Int](0).flatMap {
            ref =>
              Ref.of[IO, List[String]](List.empty).flatMap {
