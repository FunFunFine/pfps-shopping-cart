...
trait LowPriorityCatsConstraints extends TripleEquals {
  ...
  implicit def lowPriorityCatsConstraint[A, B](implicit ev1: Eq[B],
                                               ev2: A => B): CanEqual[A, B] =
...
trait CatsEquality extends LowPriorityCatsConstraints {
  ...
  override implicit def convertToCheckingEqualizer[T](left: T): CheckingEqualizer[T] =
